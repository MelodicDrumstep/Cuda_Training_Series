...
Within this file, I try to optimizhe algorithm used in "vector_add.cu".
However, I fail to exceed the performance of it.
That's because "memory coalescning", check up "README.md" for more.
...
{
    for(int idx = (threadIdx.x + blockDim.x * blockIdx.x) * compute_block_size; idx < ds && idx < (threadIdx.x + blockDim.x * blockIdx.x) * compute_block_size + compute_block_size; idx++)
    {
        C[idx] = A[idx] + B[idx];
    } 
}
...
int main(){
  ...
  vadd<<<blocks, threads>>>(d_A, d_B, d_C, DSIZE, DSIZE / (blocks * threads));
